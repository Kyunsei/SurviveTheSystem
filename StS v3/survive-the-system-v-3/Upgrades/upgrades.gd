...
@rpc("any_peer","call_remote")
func upgrade_health():
	if p.catnation_credits >=health_upgrade_cost:
		p.catnation_credits -=health_upgrade_cost
...
func upgrade_inventory_capacity():
	if p.catnation_credits >=energy_upgrade_cost:
		p.catnation_credits -=energy_upgrade_cost
		energy_upgrade_cost += 1
		p.capacity += 1.0
